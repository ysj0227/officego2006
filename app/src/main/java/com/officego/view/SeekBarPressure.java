...
public class SeekBarPressure extends View {
    ...
    private double defaultScreenHigh = 1000;  //默认后滑块位置百分比 默认
    private int setProgressMax = 1000;//设置进度条最大值 默认

    private boolean isEdit = false;     //输入框是否正在输入

    private int mTypeSeekBar;//当前是哪个seekbar
...
//        this.setBackgroundColor(Color.BLACK);
        ...
        notScrollBarBg = resources.getDrawable(R.drawable.progress_grey_line);
        hasScrollBarBg = resources.getDrawable(R.drawable.progress_main_line);
        mThumbLow = resources.getDrawable(R.mipmap.ic_slide_circle);
        mThumbHigh = resources.getDrawable(R.mipmap.ic_slide_circle);
...
//        int height = measureHeight(heightMeasureSpec);
        ...
        mOffsetLow = formatDouble(defaultScreenLow / setProgressMax * (mDistance)) + mThumbWidth / 2;
        mOffsetHigh = formatDouble(defaultScreenHigh / setProgressMax * (mDistance)) + mThumbWidth / 2;
    ...
    private int measureHeight(int measureSpec) {
        int specMode = MeasureSpec.getMode(measureSpec);
        int specSize = MeasureSpec.getSize(measureSpec);
        int defaultHeight = setProgressMax;
        //wrap_content
        if (specMode == MeasureSpec.AT_MOST) {
        }
        //fill_parent或者精确值
    ...
    protected void onDraw(Canvas canvas) {
        ...
        text_Paint.setColor(Color.parseColor("#46C3C2"));
        ...
        double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * setProgressMax / mDistance);
        double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2) * setProgressMax / mDistance);
//            Log.d(TAG, "onDraw-->mOffsetLow: " + mOffsetLow + "  mOffsetHigh: " + mOffsetHigh   + "  progressLow: " + progressLow + "  progressHigh: " + progressHigh);
        canvas.drawText((int) progressLow + "", (int) mOffsetLow - 2 - 2, 18, text_Paint);
//        canvas.drawText((int) progressHigh + "", (int) mOffsetHigh - 2, 18, text_Paint);
        canvas.drawText((int) progressHigh == setProgressMax ? getContext().getString(R.string.str_unlimited) :
                (int) progressHigh + "", (int) mOffsetHigh - 2, 18, text_Paint);

        if (mBarChangeListener != null) {
            if (!isEdit) {
                mBarChangeListener.onProgressChanged(mTypeSeekBar, this, progressLow, progressHigh);
            }
        }
    }
...
//            }
    ...
    public void setProgressLow(double progressLow) {
        ...
        mOffsetLow = formatDouble(progressLow / setProgressMax * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public void setProgressHigh(double progressHigh) {
        ...
        mOffsetHigh = formatDouble(progressHigh / setProgressMax * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }

    public void setProgressMax(int max, double percentHigh) {
        this.setProgressMax = max;
        this.defaultScreenHigh = percentHigh;
        refresh();
    }
    ...
    public void setOnSeekBarChangeListener(OnSeekBarChangeListener listener, int i) {
        mTypeSeekBar = i;
        this.mBarChangeListener = listener;
    }

    //回调函数，在滑动时实时调用，改变输入框的值
    public interface OnSeekBarChangeListener {
        //type 多个seekBar区分是哪个类型
        //滑动前
        void onProgressBefore();

        //滑动时
        void onProgressChanged(int type, SeekBarPressure seekBar, double progressLow,
                               double progressHigh);
