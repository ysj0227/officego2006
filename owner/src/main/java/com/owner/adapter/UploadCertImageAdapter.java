 ...
 * Descriptions:
 **/
public class UploadCertImageAdapter extends CommonListAdapter<ImageBean> {
    ...
    public UploadCertImageAdapter(Context context, List<ImageBean> list) {
    ...
    public interface UploadCertListener {
        ...
        void deleteCertImage(ImageBean bean, int position);
    }
    ...
    public void convert(ViewHolder holder, final ImageBean bean) {
        ...
        ivDelete.setOnClickListener(v -> {
            if (!TextUtils.isEmpty(bean.getPath())) {
                listener.deleteCertImage(bean, holder.getAdapterPosition());
            }
        });
        if (list != null && list.size() > 0 && holder.getAdapterPosition() == list.size() - 1) {
            //添加图片
            ivDelete.setVisibility(View.GONE);
            ivItem.setOnClickListener(v -> listener.addCertImage());
        } else {
